...
describe('fs-extra', () => {
  ...
  describe('+ copy()', () => {
    it.skip('should return an error if src and dest are the same', done => {
    ...
    describe('> when src is a directory', () => {
      ...
      it.skip('should preserve symbolic links', done => {
        const src = path.join(TEST_DIR, 'src')
    ...
    describe('> when filter is used', () => {
      ...
      it('should apply filter when it is applied only to dest', done => {
        ...
        const filter = (s, d) => fs.statSync(d).ctime.getTime() < timeCond
      ...
      it('should apply filter when it is applied to both src and dest', done => {
        const timeCond = new Date().getTime()
        const filter = (s, d) => s.split('.').pop() !== 'css' && fs.statSync(path.dirname(d)).ctime.getTime() > timeCond
