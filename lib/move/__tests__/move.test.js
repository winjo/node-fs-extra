...
describe('+ move()', () => {
  ...
  describe('> when overwrite = false', () => {
    ...
    it.skip('should not move a file if source and destination are the same', done => {
      const src = path.join(TEST_DIR, 'a-file')
      const dest = src
    ...
    it.skip('should not move a directory if source and destination are the same', done => {
      const src = path.join(TEST_DIR, 'a-folder')
      const dest = src
