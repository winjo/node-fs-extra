...
const os = require('os')
const atLeastNode = () => false
const fs = require('../..')
const { buffer } = require('../../external')
...
describe('fs.read()', () => {
  ...
  beforeEach(() => {
    ...
    TEST_DATA = buffer.Buffer.from(TEST_DATA.buffer)
  ...
  describe('with promises', () => {
    it('returns an object', () => {
      return fs.read(TEST_FD, buffer.Buffer.alloc(SIZE), 0, SIZE, 0)
        .then(results => {
    ...
    it('returns an object when position is not set', () => {
      return fs.read(TEST_FD, buffer.Buffer.alloc(SIZE), 0, SIZE)
        .then(results => {
          const bytesRead = results.bytesRead
          const buffer = results.buffer
          assert.strictEqual(bytesRead, SIZE, 'bytesRead is correct')
  ...
  describe('with callbacks', () => {
    it('works', done => {
      fs.read(TEST_FD, buffer.Buffer.alloc(SIZE), 0, SIZE, 0, (err, bytesRead, buffer) => {
        assert.ifError(err)
        assert.strictEqual(bytesRead, SIZE, 'bytesRead is correct')
        assert(buffer.equals(TEST_DATA), 'data is correct')
        done()
    ...
    it('works when position is null', done => {
      fs.read(TEST_FD, buffer.Buffer.alloc(SIZE), 0, SIZE, null, (err, bytesRead, buffer) => {
        assert.ifError(err)
        assert.strictEqual(bytesRead, SIZE, 'bytesRead is correct')
        assert(buffer.equals(TEST_DATA), 'data is correct')
        done()
...
describe('fs.write()', () => {
  ...
  describe('with promises', () => {
    ...
    it('returns an object when minimal arguments are passed', () => {
      return fs.write(TEST_FD, TEST_DATA, 0, SIZE)
        .then(results => {
          const bytesWritten = results.bytesWritten
          const buffer = results.buffer
          assert.strictEqual(bytesWritten, SIZE, 'bytesWritten is correct')
    ...
    it('returns an object when writing a string', () => {
      ...
      return fs.write(TEST_FD, message)
        .then(results => {
          ...
          assert.strictEqual(buffer.toString('utf8'), message, 'data is correct')
        })
    })
  })
  ...
  describe('with callbacks', () => {
    ...
    it.skip('works when minimal arguments are passed', done => {
    ...
    it.skip('works when writing a string', done => {
      const message = 'Hello World!'
